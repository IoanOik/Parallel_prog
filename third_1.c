...
int main(int argc, char const *argv[])
{
    ...
    #pragma omp parallel firstprivate(gen)
    {
        while (true)
        {
            ...
            // the 70% best will be allowed to reproduce
            #pragma omp for
    ...
    printf("organism: %s sore: %d percent: %0.2f generation: %d\n", population[0].chromosome, population[0].score, percent_total, final_generation);
    printf("\n\nExecution time: %f\n\n", t2 - t1);
    return 0;
}
